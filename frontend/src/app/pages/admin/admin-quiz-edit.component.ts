...
export class AdminQuizEditComponent {
  ...
  async updateQuiz(form: NgForm) {
    ...
    const current = this.data.quizzes().find((item: Quiz) => item.id === this.quizId);
    if (!current) {
      this.formError.set('Quiz not found.');
      return;
    }
  ...
  private async loadQuiz(id: string) {
    try {
      ...
      this.draft = {
        ...
        status: quiz.status,
        publishAt: quiz.publishAt ?? this.defaultPublishAt()
      };
  ...
  private extractErrorMessage(error: unknown) {
    ...
    return 'The quiz could not be updated. Check that the backend services are running.';
  }
}
